...
Generated at 2025-10-16T04:54:36.723906+00:00
...
- `scripts/selftest_loop.sh` resets `.selftest_workspace/`, installs the current checkout, runs two Feature Cards (`hello_add`, `hello_mul`) through generator → discriminator, appends logs/status/spec listings/runtime code to the latest audit file, then removes the workspace (`SELFTEST_KEEP=1` preserves it for debugging).
- `scripts/smoke_e2e.sh` spins up a temp repo, installs the current checkout via `scripts/install.sh`, scaffolds a Feature Card, runs `./rex-codex loop --feature-only`, then executes the global discriminator sweep. Export `KEEP=1` while debugging to retain the workspace.
...
- `scripts/selftest_loop.sh` – fast offline selftest that resets `.selftest_workspace/`, runs two feature cards, and appends logs/status/spec listings to the latest audit file (`SELFTEST_KEEP=1` preserves the workspace).
...
- `scripts/selftest_loop.sh` resets `.selftest_workspace/`, installs the current checkout, exercises two feature cards (`hello_add`, `hello_mul`) via generator → discriminator, appends the command log/status/spec listing/runtime code to the latest audit file, then removes the workspace (set `SELFTEST_KEEP=1` to inspect).
- `scripts/smoke_e2e.sh` creates a throwaway repo, installs the current checkout via `scripts/install.sh`, scaffolds a Feature Card, and runs `./rex-codex loop --feature-only` followed by the global discriminator sweep (`KEEP=1` preserves the temp repo).
...
declare -a SLUGS=("hello_add" "hello_mul")
declare -A TITLES
declare -A SUMMARIES
declare -A ACCEPTANCE

TITLES["hello_add"]="Add integers"
SUMMARIES["hello_add"]="Provide hello.add that sums two integers."
ACCEPTANCE["hello_add"]="hello.add(2,3) returns 5"

TITLES["hello_mul"]="Multiply integers"
SUMMARIES["hello_mul"]="Provide hello.multiply that multiplies integers."
ACCEPTANCE["hello_mul"]="hello.multiply(2,3) returns 6"
...
./rex-codex card new hello \
  --title "Hello feature" \
  --summary "Provide hello.add that adds two ints." \
